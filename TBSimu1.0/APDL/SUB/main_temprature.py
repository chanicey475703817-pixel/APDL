...
from myalphashape.main_temprature import alphashape_fileio
...
from myalphashape.main_temprature import alphamain
# from models.log_write import log_write_and_interrupt

project_root = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB"
# 原始APDL批处理文件路径
original_square = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\model_temprature_square.mac"
original_cylinder = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\model_temprature_cylinder.mac"
original_lshape = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\model_temprature_lshape.mac"
...
# 临时文件保存的文件夹路径
temp_folder_path = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2"
...
def single_run(t_value, mapdl, failure_log="failures.log"):  #输入t值后，对三种模型各自处理，进行一次模拟并输出边界点ID文件csv，接下来进行绘图
    ...
    for shape,path in test_list.items():        #三种形状均遍历一遍,暂时注释掉用于固定输出
        ...
        output_folder = r"C:\\mine\\apdl\\ThermalSimulation\\TBSimu1.0\\APDL\\SUB\\TEMP2\\OUTLINE"
        output_name = f"{shape}_TO_{t_value}_data.csv"
        output_path = os.path.join(output_folder,output_name)
        csv_output = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_csv\TEMP_Node_Data.csv"#原始输出文件
    ...
    #建模及轴侧视角出图部分
        twod_folder = "C:\\mine\\apdl\\ThermalSimulation\\TBSimu1.0\\APDL\\SUB\\TEMP2\\2DPOINTCLOUD"
        twod_path = os.path.join(twod_folder,output_name)
        threed_folder = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\POINTCLOUD"
        threed_path = os.path.join(threed_folder,output_name)
    #文件修改部分
        
        #batch脚本保存地址
        #temp_2dvread = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_batch\temp_2d.mac"
        temp_3dvread = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_batch\temp_3d.mac"
        temp_boundary = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_batch\temp_boundary.mac"
        #temp_2dplot = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_batch\temp_2Dplot.mac"
        #temp_3dplot = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\temp_batch\temp_3Dplot.mac"
        Fig_path = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\FIG"
...
def main_loop( t=-1, failure_log="failures.log"):  # k=-1, tbh=-1, tbw=-1
    ...
    # 设置默认范围
    t_range = [t] if t != -1 else [round(i / 10.0, 2) for i in range(50, 505, 10)] #便于调试时使用单个参数，运行时自动扫描。这里使用了列表推导式
