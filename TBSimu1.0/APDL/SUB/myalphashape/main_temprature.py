...
#聚类方法，难以控制
# def define_effected_points1(df,col,k):  
#     df0 = df[col].tolist()
#     print(f"数据长度: {len(df0)}")
#     X = np.array(df0).reshape(-1, 1)
#     print(f"聚类数据形状: {X.shape}")

#     kmeans = KMeans(n_clusters=k)
#     kmeans.fit(X)

#     indices_label_1 = np.where(kmeans.labels_ == 1)[0]
    
#     return indices_label_1

def filter_points(input_csv,output_csv,THREED_NAME,q_0):        #热流密度矢量定义函数
    ...
    # 假设输入 CSV 列名如下：NODE_ID, X, Y, Z, TEMP
    ...
    df = df.sort_values(by=['Z'], ascending=False).reset_index(drop=True)
    start = 0
    cut = [0]  # 初始化cut列表
    ...
    #按得到的cut对df进行分段调取
    # for i in range(len(cut) - 1):  #这里直接选取z=0的点
    z_part = df.iloc[cut[0]:cut[1]].copy()
    # 5. 应用公式中的条件进行逻辑筛选 - 使用HF_SUM列筛选异常值
    outlier_index = define_effected_points(z_part,'TEMP',q_0)
    # outlier_index = define_effected_points(z_part,'TEMP',q_0)

    # 为z_part创建条件 - 筛选HF_SUM的异常值
    condition = z_part.index.isin(outlier_index)

    # 6. 筛选出满足公式条件的点并加入列表
    filtered_points = z_part[condition]
    ...
    pointcloud = r"C:\mine\apdl\ThermalSimulation\TBSimu1.0\APDL\SUB\TEMP2\POINTCLOUD"
...
def alphamain(input,output,twod_name,threed_name,fig_boolean=False,alpha=0.5,q_0=0.05):


    #筛点函数中转路径
    
    temp_path_folder = "C:\\mine\\apdl\\ThermalSimulation\\TBSimu1.0\\APDL\\SUB\\TEMP2\\2DPOINTCLOUD"
